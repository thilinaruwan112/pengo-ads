...
export function EditCampaignForm({ campaign }: EditCampaignFormProps) {
  ...
  const handleSave = async () => {
    ...
    try {
        const account = accounts.find(acc => acc.campaigns.some(c => c.id === campaign.id));
        if (!account) throw new Error("Could not find account for this campaign.");

        const campaignIndex = account.campaigns.findIndex(c => c.id === campaign.id);
        if (campaignIndex === -1) throw new Error("Could not find this campaign.");
        
        account.campaigns[campaignIndex] = { ...account.campaigns[campaignIndex], ...updatedCampaignData };

        toast({
            title: "Campaign Updated",
            description: "Your campaign has been successfully saved. (Note: Changes are in-memory).",
        });
        ...
        router.refresh(); // Refresh server components
         ...
         toast({
            title: "Error",
            description: error.message || "Could not save campaign changes.",
            variant: "destructive"
        });
    } finally {
