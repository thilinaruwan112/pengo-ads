...
export function EditCompanyForm({ company, isNew, clients }: EditCompanyFormProps) {
  ...
  const handleSave = async () => {
    ...
    try {
        let savedCompany: Account;

        if (isNew) {
            savedCompany = {
                id: `act_${Math.random().toString(36).substr(2, 9)}`,
                name: `${clientName}'s Account`,
                accessToken: `FAKE_TOKEN_${Math.random()}`,
                campaigns: [],
                ...companyData,
            } as Account;
            mockAccounts.push(savedCompany);
        } else {
            const companyIndex = mockAccounts.findIndex(a => a.id === company!.id);
            if (companyIndex === -1) throw new Error("Company not found for update");
            
            mockAccounts[companyIndex] = { ...mockAccounts[companyIndex], ...companyData };
            savedCompany = mockAccounts[companyIndex];
        }
        ...
        if (initialClientId !== selectedClientId) {
            ...
            if (initialClientId) {
                ...
                if (oldClient) {
                    oldClient.adAccountIds = oldClient.adAccountIds.filter(id => id !== savedCompany.id);
                }
            }
             // Add to new client
            const newClient = clients.find(c => c.id === selectedClientId);
             if (newClient) {
                newClient.adAccountIds.push(savedCompany.id);
             }
        }
        
        toast({
            title: isNew ? "Company Created" : "Company Updated",
            description: `The company details have been successfully saved. (Note: Changes are in-memory).`,
        });
